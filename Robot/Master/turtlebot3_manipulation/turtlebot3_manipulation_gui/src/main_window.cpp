...
void MainWindow::on_btn_send_joint_angle_clicked(float& a, float& b, float& c, float& d)
{
  std::vector<double> joint_angle;
  double path_time = ui.doubleSpinBox_time_js->value();

  joint_angle.push_back(a);
  joint_angle.push_back(b);
  joint_angle.push_back(c);
  joint_angle.push_back(d);
